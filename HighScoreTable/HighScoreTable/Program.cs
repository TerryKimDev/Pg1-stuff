...
namespace HighScoreTable
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            int Scores= 0;
            Console.Write("Please input a how many high scores to store: ");
            while (!int.TryParse(Console.ReadLine(), out Scores) || Scores < 0)
            {
            ...
            Console.Clear();
            Console.WriteLine("You will now input the value of each score.");
            for (int i = 0; i < highScores.Length; i++)
            {
                Console.Write("score #"+ (i +1) +" : ");
                highScores[i] = int.Parse(Console.ReadLine());
            }
